/**
 * BIOMARKERS LIST CARD COMPONENT
 * 
 * Displays all biomarkers with status, value, and explanations
 */
...
interface Biomarker {
  ...
  lastMeasuredAt?: string; // YYYY-MM-DD or "No medido aún"
  measurementCount?: number; // Number of historical measurements
}
...
export default function BiomarkersListCard({ biomarkers, hasBaseline = false }: BiomarkersListCardProps) {
  ...
  // Trend icons only shown when hasBaseline is true
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="space-y-4">
        {biomarkers.map((biomarker) => (
          <div
            key={biomarker.id}
            className="border-2 border-gray-200 rounded-lg p-4 hover:border-primary-300 transition"
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className={`w-4 h-4 rounded-full ${getTrafficLightColor(biomarker.traffic_light)}`} />
              ...
              <div className="text-right">
                ...
                {/* FECHA DE ÚLTIMA MEDICIÓN (OBLIGATORIO) */}
                ...
                {/* COMPARACIÓN (SOLO SI HAY DATOS) */}
                {/* REGLA: Solo mostrar comparación si este biomarcador tiene ≥2 mediciones */}
            ...
            {/* Cada biomarcador muestra: valor, estado (color), fecha última medición, tendencia SOLO si existe */}
            <div className="mb-3">
